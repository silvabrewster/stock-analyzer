...
def save_scan_to_db(df, market: dict):
    ...
    vix = market.get("vix", {})
    tny = market.get("tny", {})
    ai_brief_val = market.get("ai_brief", "")
    conn.execute("""
...
def dashboard():
    ...
    # AI brief from latest scan (stored in DB or generated)
    ai_brief = None
    try:
...
def earnings_calendar():
    ...
    from analyzer import build_universe
    conn     = get_db()
    universe = build_universe()

    # Get scores for tickers
    scores = {}
    try:
        rows = conn.execute(
            "SELECT ticker, score FROM scans WHERE scan_date = (SELECT MAX(scan_date) FROM scans)"
        ).fetchall()
        for r in rows:
    ...
    earnings = get_earnings_calendar(universe[:25])  # limit for speed
    for e in earnings:
