...
_scan_status = {"running": False, "done": False, "error": None}

@app.route("/momentum/scan", methods=["POST"])
@login_required
def momentum_scan():
    """Triggers penny stock scan in background thread."""
    import threading
    global _scan_status

    if _scan_status.get("running"):
        return render_template("momentum_scanning.html")

    _scan_status = {"running": True, "done": False, "error": None}
    ...
    def run_scan():
        ...
        try:
            ...
            conn.commit()
            conn.close()
            _scan_status = {"running": False, "done": True, "error": None}
            print("✓ Penny scan complete")
        except Exception as e:
            _scan_status = {"running": False, "done": False, "error": str(e)}
