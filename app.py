...
v3.2 additions:
  - Market regime detection on dashboard
  - Portfolio optimization on /portfolio
  - Signal alignment shown on stock detail
"""
...
def init_db():
    ...
    tables = [
        """CREATE TABLE IF NOT EXISTS scans (
            ...
            streak INTEGER, is_new TEXT, sector TEXT,
            alignment TEXT,
...
def save_scan_to_db(df, market: dict):
    ...
    """, (today, sp.get("price"), sp.get("chg"), vix.get("price"), tny.get("price"),
          ...
          market.get("regime_label",""),
          market.get("regime_confidence",0)))
...
def dashboard():
    ...
    # Market regime — compute from stored data (no API calls)
    regime = None
    try:
    ...
    # Alert checks (independent connection)
    try:
...
def api_analyze(ticker):
    ...
    try:
        ...
        try:
            ...
            price = fi.last_price or fi.regular_market_price or 0
        except Exception:
        ...
        alignment = {}
        try:
            from analyzer import get_signal_alignment
            alignment = get_signal_alignment(ticker.upper())
        except Exception:
            pass
...
- Smart Buy Rating: {smart_buy['label']}
- Signal Alignment: {alignment.get('label', 'N/A')}
...
def stock_detail(ticker):
    ...
    live_price = None
    try:
        from features import get_cached_price, init_price_cache
        init_price_cache(conn)
        live_price = get_cached_price(conn, ticker, max_age_minutes=20)
        conn.commit()
    except Exception:
        pass

    # Signal alignment from DB (stored during scan) or compute live
    alignment = None
    try:
        if latest and latest.get("alignment"):
            alignment = {"label": latest["alignment"], "emoji": "🎯" if "Full" in (latest["alignment"] or "") else ""}
        else:
            from analyzer import get_signal_alignment
            alignment = get_signal_alignment(ticker)
    except Exception:
        pass
...
def watchlist():
    ...
    for w in items:
        ...
        current_price = get_cached_price(conn, ticker, max_age_minutes=20)
    ...
    for row in rows:
        try:
            ...
            if price:
                ...
                conn.execute("INSERT INTO price_cache (ticker,price,fetched_at) VALUES (?,?,?) ON CONFLICT(ticker) DO UPDATE SET price=excluded.price,fetched_at=excluded.fetched_at", (row["ticker"], price, datetime.now().isoformat()))
                result.append({"ticker":row["ticker"],"current_price":price,"current_value":round(row["shares"]*price,2)})
        except Exception:
            pass
    conn.commit(); conn.close()
...
def api_push_subscribe():
    try:
        data=request.get_json(); endpoint=data.get("endpoint",""); p256dh=data.get("keys",{}).get("p256dh",""); auth=data.get("keys",{}).get("auth","")
