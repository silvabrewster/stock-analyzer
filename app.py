...
def save_scan_to_db(df, market: dict):
    ...
    conn.execute("""
        ...
        VALUES (?,?,?,?,?,?)
    """, (today, sp.get("price"), sp.get("chg"), vix.get("price"), tny.get("price"), market.get("ai_brief","")))
...
def stock_detail(ticker):
    ...
    # Smart buy from DB data (no yfinance call needed — fast!)
    smart_buy = None
    if latest:
        from features import get_smart_buy_rating
        smart_buy = get_smart_buy_rating(
            score       = latest.get("score"),
            upside_pct  = latest.get("upside_pct"),
            short_pct   = latest.get("short_pct"),
            week52_pos  = latest.get("week52_pos"),
            beta        = latest.get("beta"),
            price       = latest.get("price"),
            price_target= None  # not stored in DB, so skip target check here
        )

    # Live price from cache (fast — no blocking yfinance call)
    live_price = None
    try:
    ...
    return render_template("stock_detail.html",
        ...
        earnings_warning=None,   # removed — was slow, TradingView chart shows this
        smart_buy=smart_buy,
...
def watchlist():
    """
    Watchlist now uses cached prices — no slow yfinance .info calls on load.
    Prices refresh every 20 minutes automatically.
    """
    conn = get_db()
    from features import get_cached_price, init_price_cache
    init_price_cache(conn)
    ...
    for w in items:
        ...
        # Fast cached price — no blocking call
...
def portfolio():
    """
    Portfolio now uses cached prices — no slow yfinance .info per holding.
    """
    conn = get_db()
    from features import get_cached_price, init_price_cache
    init_price_cache(conn)
    ...
    return render_template("portfolio.html",
        ...
        avg_score=avg_score, alerts=None)
...
def api_portfolio_prices():
    """Refresh portfolio prices — clears cache and re-fetches live."""
    import yfinance as yf
    conn = get_db()
    ...
    for row in rows:
        ...
        try:
            ...
            if price:
                ...
                result.append({"ticker": ticker, "current_price": price, "current_value": round(shares*price,2)})
        except Exception:
            pass
...
def earnings_calendar():
    ...
    from features import get_earnings_calendar      # now parallel — much faster
