...
# How long the per-ticker Yahoo loop may run before we give up and keep what we
# have. Without this the scan can outlive the host and never reach the save step.
YAHOO_LOOP_BUDGET_SEC = 240


def _yahoo_available() -> bool:
    """One cheap probe before walking every ticker.

    Yahoo blocks some hosts outright (Render's shared IPs among them). When that
    happens each of the ~92 lookups still burns retries and sleeps, so the scan
    runs for far longer than the host stays alive and nothing is ever saved.
    Probing once lets us skip straight to the degraded path.
    """
    try:
        fi = yf.Ticker("AAPL").fast_info
        return bool(fi.last_price or fi.regular_market_price)
    except Exception:
        return False


def _stooq_price(ticker: str):
    """Current price from Stooq — unrelated to Yahoo, so it survives the block."""
    try:
        symbol = ticker.lower().replace("-", ".") + ".us"
        r = requests.get(f"https://stooq.com/q/l/?s={symbol}&f=sd2t2ohlcvn&h&e=csv",
...
def _stooq_prices_bulk(tickers: list) -> dict:
    """Fetch many Stooq prices at once; serial fetches would take minutes."""
    from concurrent.futures import ThreadPoolExecutor, as_completed
    prices = {}
    try:
        with ThreadPoolExecutor(max_workers=8) as ex:
            futures = {ex.submit(_stooq_price, t): t for t in tickers}
...
SECTOR_MAP = {
    ...
    # Remainder of the scan universe. Yahoo supplies sectors when it is reachable,
    # but it is blocked from some hosts, and without these the dashboard shows
    # "Unknown" for half the table.
    "ACN":"Technology","TXN":"Technology","IBM":"Technology","AMAT":"Technology",
    "MU":"Technology","KLAC":"Technology","LRCX":"Technology","ADI":"Technology",
    "SNPS":"Technology","CDNS":"Technology","PTC":"Technology","FTNT":"Technology",
    "OKTA":"Technology","DDOG":"Technology","MDB":"Technology","APP":"Technology",
    "SPGI":"Financial Services","SCHW":"Financial Services","MCO":"Financial Services",
    "HOOD":"Financial Services","COIN":"Financial Services",
    "SYK":"Healthcare","GILD":"Healthcare","REGN":"Healthcare","VRTX":"Healthcare",
    "ZTS":"Healthcare","IQV":"Healthcare","EW":"Healthcare","BSX":"Healthcare",
    "MDT":"Healthcare","AMGN":"Healthcare","BIIB":"Healthcare","ILMN":"Healthcare",
    "IDXX":"Healthcare","ALGN":"Healthcare","RMD":"Healthcare","DXCM":"Healthcare",
    "PODD":"Healthcare","A":"Healthcare",
    "TT":"Industrials","ETN":"Industrials","PH":"Industrials","ROK":"Industrials",
}
...
def get_yahoo_strong_buys(tickers: list) -> dict:
    ...
    # If Yahoo is blocking this host, walking every ticker would take longer than
    # the host stays awake and the scan would never save. Degrade instead: keep
    # sectors from the static map and get prices from Stooq, which still answers.
    if not _yahoo_available():
        print("  → Yahoo unreachable from this host — skipping per-ticker lookups")
        print("  → Falling back to Stooq for prices...")
        prices = _stooq_prices_bulk(tickers)
        for ticker in tickers:
            results[ticker] = {
                "yahoo_strong_buy": False,
                "sector":           SECTOR_MAP.get(ticker, "Unknown"),
                "current_price":    prices.get(ticker),
                "yahoo_unavailable": True,
            }
        print(f"  → {len(prices)}/{len(tickers)} prices from Stooq")
        print("  → 0 Yahoo Strong Buys found (source unavailable)")
        return results

    loop_started = time.time()
    for ticker in tickers:
        if time.time() - loop_started > YAHOO_LOOP_BUDGET_SEC:
            print(f"  → Time budget reached; keeping {len(results)} of {len(tickers)}")
            for remaining in tickers:
                results.setdefault(remaining, {
                    "yahoo_strong_buy": False,
                    "sector": SECTOR_MAP.get(remaining, "Unknown"),
                })
            break
        try:
...
def get_morningstar_ratings(tickers: list, yahoo_data: dict = None) -> dict:
    ...
    for ticker in tickers:
        try:
            ...
            # Fall back to a quick .info fetch only if we have nothing — but never
            # when the probe already told us Yahoo is blocked, or this loop would
            # burn retries on all ~92 tickers and outlive the host.
            info = {}
            if not current and not y.get("yahoo_unavailable"):
                try:
            ...
            # The sleep only exists to pace Yahoo requests; skip it when we made none.
            if not y.get("yahoo_unavailable"):
                time.sleep(0.3)
        except Exception:
