...
def get_vanguard_top_holdings() -> dict[str, float]:
    ...
    for etf_ticker in VANGUARD_ETFS:
        try:
            ...
            # yfinance returns top holdings as a DataFrame
            df = etf.funds_data.top_holdings if hasattr(etf, "funds_data") else None

            if df is None or (hasattr(df, "empty") and df.empty):
                # Alternative path
                info = etf.info
                # yfinance sometimes exposes holdings in info
                continue
